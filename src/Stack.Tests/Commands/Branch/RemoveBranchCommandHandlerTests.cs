    ...
    public async Task WhenBranchHasChildren_StillAsksForChildAction()
    {
        ...
        var stackConfig = new TestStackConfigBuilder()
            .WithStack(stack => stack
                ...
                .WithRemoteUri(remoteUri)
                .WithSourceBranch(sourceBranch)
                .WithBranch(b => b.WithName(branchToRemove).WithChildBranch(b => b.WithName(childBranch)))) // Branch with children
            .Build();
        var inputProvider = Substitute.For<IInputProvider>();
        var logger = XUnitLogger.CreateLogger<RemoveBranchCommandHandler>(testOutputHelper);
        var gitClient = Substitute.For<IGitClient>();
        ...
        // Should ask for child action when branch has children
        await inputProvider.Received(1).Select(Questions.RemoveBranchChildAction, Arg.Any<RemoveBranchChildAction[]>(), Arg.Any<CancellationToken>(), Arg.Any<Func<RemoveBranchChildAction, string>>());
    }
}
